    ...
    <>
      {movieDetails?.Response.toLowerCase() === 'true' ? (
        <div className='movie-result-block' key={`movie-details-${movieDetails.imdbID}`}>
          ...
          <p title={`Languages: ${movieDetails?.Language}`} aria-label={`Languages: ${movieDetails?.Language}`}>Languages: {movieDetails?.Language}</p>
          <p title={`Countries: ${movieDetails?.Country}`} aria-label={`Countries: ${movieDetails?.Country}`}>Countries: {movieDetails?.Country}</p>
          <div className='movie-result-img-block'>
            <img src={movieDetails?.Poster.startsWith('http') ? movieDetails?.Poster : NO_IMG} width='100%' style={{ alignContent: 'start' }} alt={`Movie - ${movieDetails?.Title} Released in ${movieDetails?.Released}`} />
          </div>
          <div>
            <p>
            ...
            <hr />
            <p title={`Directors: ${movieDetails?.Director}`} aria-label={`Directors: ${movieDetails?.Director}`}><b>Director: </b><br />{movieDetails?.Director}</p>
            <p title={`Writers: ${movieDetails?.Director}`} aria-label={`Writers: ${movieDetails?.Director}`}><b>Writers:</b><br />{movieDetails?.Writer}</p>
            <p title={`Actors: ${movieDetails?.Director}`} aria-label={`Actors: ${movieDetails?.Director}`}><b>Actors:</b><br />{movieDetails?.Actors}</p>
          </div>
          <hr />
          <div className="row center-text">
            <div className="col">
              <p className='ratings'>IMDB Rating</p>
              <p title={`IMDB Rating: ${movieDetails?.imdbRating}`} aria-label={`IMDB Rating: ${movieDetails?.imdbRating}`}>{movieDetails?.imdbRating}</p>
            </div>
            <div className="col">
